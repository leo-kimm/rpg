...
export const FARM_PLOT_ZONES = [
  { x: 32, y: 5, w: 3, h: 2 },  // 1구역 (기존)
  { x: 36, y: 5, w: 3, h: 2 },  // 2구역 (기존)
  { x: 40, y: 5, w: 3, h: 2 },  // 3구역 (신규)
  { x: 44, y: 5, w: 3, h: 2 }   // 4구역 (신규)
];

export function isFarmableTile(tx, ty) {
  const sectors = [
    { x: [32, 34], y: [5, 6] }, { x: [36, 38], y: [5, 6] },
    { x: [40, 42], y: [5, 6] }, { x: [44, 46], y: [5, 6] }
  ];
  return sectors.some(s => tx >= s.x[0] && tx <= s.x[1] && ty >= s.y[0] && ty <= s.y[1]);
}
